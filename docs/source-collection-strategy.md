...
추가 확인 결과 Hyperconnect와 NHN Cloud는 RSS보다 sitemap 수집이 더 적합했습니다.

| Source | RSS 수집량 | 확장 경로 | 확장 수집량 | 최종 전략 |
| --- | ---: | --- | ---: | --- |
| `hyperconnect-tech-blog` | 10 | `https://hyperconnect.github.io/sitemap.xml` | 113 | `sitemap / none / article_fetch` |
| `nhn-cloud-meetup` | 10 | `https://meetup.nhncloud.com/sitemap.xml` | 420 | `sitemap / none / article_fetch` |
| `watcha-tech-blog` | 10 | Medium archive | 10 | `rss / none / feed_only` 유지 |

Watcha는 `https://medium.com/watcha/archive` 요청이 403으로 차단되어 이번 라운드에서는 최신 10개 RSS 수집만 유지했습니다.
...
대량 article 유입으로 검색 후보 풀이 넓어졌지만, 평가셋은 아직 신규 source의 AI/추천/보안/ClickHouse/클라우드 운영 사례를 반영하지 못합니다. source 확장 후에는 평가셋 보강을 함께 진행합니다.

확장 수집 후 Hyperconnect/NHN Cloud의 신규 article을 100개 단위로 LLM 요약했습니다.

| Source | 추가 요약 결과 |
| --- | ---: |
| `hyperconnect-tech-blog` | 103 |
| `nhn-cloud-meetup` | 410 |

NHN Cloud 요약 중 1건은 read timeout이 발생했지만 다음 배치에서 재시도되어 최종 성공했습니다.
...
요약 반영으로 검색 문서의 `caseSummary`, `caseProblem`, `caseSolution`, `technologies`, `problemKeywords`가 보강되면서 검색 평가가 일부 회복되었습니다. 단, NHN Cloud 일부 글은 title 추출이 URL로 저장되어 있어 article fetcher의 title parsing 보정이 필요합니다.
